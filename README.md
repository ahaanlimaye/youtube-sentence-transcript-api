# YouTube Sentence Transcript API

This is a fork of [youtube-transcript-api](https://github.com/jdepoix/youtube-transcript-api) that provides sentence-level transcript processing with improved timing accuracy. It allows you to get the transcripts/subtitles for a given YouTube video, with the following improvements:

- Sentence-level transcript processing
- More accurate timing information
- Better handling of multilingual content
- Improved sentence boundary detection
- No empty snippets in the output

## Installation

```bash
pip install youtube-sentence-transcript-api
```

## Usage

```python
from youtube_sentence_transcript_api import YouTubeTranscriptApi

# Get transcript for a video
transcript = YouTubeTranscriptApi().fetch('video_id')

# Print each sentence with its timing
for snippet in transcript:
    print(f"Text: {snippet.text}")
    print(f"Start: {snippet.start:.2f}s")
    print(f"Duration: {snippet.duration:.2f}s")
    print("-" * 50)
```
...
- Supports translation of transcripts
- No headless browser required
- Sentence-level processing for better readability
...
This project is a fork of [youtube-transcript-api](https://github.com/jdepoix/youtube-transcript-api) by Jonas Depoix.
